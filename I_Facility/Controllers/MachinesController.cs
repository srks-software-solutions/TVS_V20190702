...
namespace I_Facility.Controllers
{
    public class MachinesController : Controller
    {
        ...
        [HttpGet]
        public ActionResult MachineList(/*int message = 0*/)
        {
            ...
            //if (message==0)
            //{
            //    ViewBag.AlertMessage = "Asset Code is already existed";
            //}
            return View(ma);
        ...
        public ActionResult CreateMachine(MachineModel tblmachine, int dept = 0, int Plant = 0, int cell = 0, int Machnecategory = 0)
        {
            ...
            ViewBag.Plant = new SelectList(db.tblplants.Where(m => m.IsDeleted == 0), "PlantID", "PlantName", tblmachine.Machine.PlantID);
            ViewBag.dept = new SelectList(db.tblshops.Where(m => m.IsDeleted == 0), "ShopId", "ShopName", tblmachine.Machine.ShopID);
            ViewBag.cell = new SelectList(db.tblcells.Where(m => m.IsDeleted == 0), "CellID", "CellName", tblmachine.Machine.CellID);
            ViewBag.Logout = Session["Username"].ToString().ToUpper();
            ViewBag.roleid = Session["RoleID"];

            string machinename = tblmachine.Machine.MachineName;
            var asset = tblmachine.Machine.AssetCode;
            var duplicateEntry = db.tblmachinedetails.Where(m => m.IsDeleted == 0 && m.PlantID == Plant && m.ShopID == dept && m.CellID == Machnecategory && m.MachineName == machinename && m.IsBottelNeck == tblmachine.Machine.IsBottelNeck && m.IsFirstMachine == tblmachine.Machine.IsFirstMachine && m.IsLastMachine == tblmachine.Machine.IsLastMachine).ToList();
            if (duplicateEntry.Count == 0)
            {
                try
                {
                    ...
                    var assetValue = (from s in db.tblmachinedetails where s.IsDeleted == 0 select s.AssetCode).ToList();
                    foreach (var item in assetValue)
                    {
                        if (item == asset)
                        {
                            //ModelState.AddModelError("AssetCode", @"AssetCode already exists");
                            TempData["Message"] = "Asset Code is already existed";
                            return View();
        ...
        public ActionResult EditMachine(tblmachinedetail tblmachine, int dept = 0, int plant = 0, int cell = 0, string Machine = "")
        {
            ...
            ViewBag.cell = new SelectList(db.tblcells.Where(m => m.IsDeleted == 0), "CellID", "CellName", tblmachine.CellID);
            ViewBag.Logout = Session["Username"].ToString().ToUpper();
            ViewBag.roleid = Session["RoleID"];
            ...
            var duplicateEntry = db.tblmachinedetails.Where(m => m.IsDeleted == 0 && m.PlantID == plant && m.ShopID == dept && m.CellID == tblmachine.CellID && m.MachineName == tblmachine.MachineName && m.MachineID != tblmachine.MachineID && m.IsLastMachine == tblmachine.IsLastMachine && m.IsFirstMachine == tblmachine.IsFirstMachine && m.IsBottelNeck == tblmachine.IsBottelNeck).ToList();
            if (duplicateEntry.Count == 0)
            {
                ...
                MachineDet.AssetCode = tblmachine.AssetCode;
                MachineDet.OperationNumber = tblmachine.OperationNumber;
                MachineDet.IsBottelNeck = tblmachine.IsBottelNeck;
                MachineDet.IsFirstMachine = tblmachine.IsFirstMachine;
                MachineDet.IsLastMachine = tblmachine.IsLastMachine;
                ...
                //var sa = MachineDet.AssetCode;
                var asset = (from s in db.tblmachinedetails where s.IsDeleted == 0 && s.MachineID != tblmachine.MachineID select s.AssetCode).ToList();
                foreach (var item in asset)
                {
                    if (item == MachineDet.AssetCode)
                    {
                        //ModelState.AddModelError("AssetCode", @"AssetCode already exists");
                        TempData["Message"] = "Asset Code is already existed for this tblmachine.MachineID";
                        return View();
                        //return RedirectToAction("MachineList");
        ...
        //    var asset = (from s in db.tblmachinedetails where s.IsDeleted == 0 select s.AssetCode).ToList();
        //    foreach (var item in asset)
        //    {
        ...
        //        {
        //            status = "Exist";
        //            //ViewBag.Message = "Asset Code is already existed";
        //        }
        //        else
        //        {
        //            status = "NotExist";
