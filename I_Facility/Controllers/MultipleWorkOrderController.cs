...
namespace I_Facility.Controllers
{
    public class MultipleWorkOrderController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            ViewBag.ShopID = new SelectList(db.tblshops.Where(m => m.IsDeleted == 0), "ShopID", "ShopName");
            ViewBag.CellID = new SelectList(db.tblcells.Where(m => m.IsDeleted == 0), "CellID", "CellName");
            ...
            mplist = db.tblmultipleworkorders.Where(m => m.IsDeleted == 0).ToList();
        ...
        public ActionResult Create(MultiWorkOrderModel tee, int Plant = 0, int CellID = 0, int ShopID = 0, int MachineID = 0)
        {
            ...
            db.tblmultipleworkorders.Add(tee.Multiworkorder);
            db.SaveChanges();
        ...
        {
            var Data = db.tblmultipleworkorders.Where(m => m.MWOID == Id).Select(m => new { Mname = m.MultipleWOName, Mdesc = m.MultipleWODesc, PlantID = m.PlantID, ShopID = m.ShopID, CellID = m.CellID, Macid = m.WCID }).ToList();
        ...
        public ActionResult Edit(MultiWorkOrderModel tee, int Plant = 0, int CellID = 0, int ShopID = 0, int MachineID = 0)
        {
            ...
            tee.Multiworkorder.IsDeleted = 0;
            tee.Multiworkorder.IsEnabled = 1;
            db.Entry(tee.Multiworkorder).State = EntityState.Modified;
            db.SaveChanges();
            return RedirectToAction("Index");
        }
        ...
        public List<int> MWO_OverlapCheckerForPlantDownwards(int plantid, int mwoID = 0)
        {
            ...
            if (overlappingPlanId.Count == 0)
            {
                ...
                if (mwoID != 0)
                {
                    sql = "SELECT MWOID FROM i_facility.tblmultipleworkorder where PlantID = '" + PlantID + "' and ShopID is null and CellID is null and WCID is null and IsDeleted = 0 and MWOID != " + mwoID + " ;";
                }
                else
                {
                    sql = "SELECT MWOID FROM i_facility.tblmultipleworkorder where PlantID = '" + PlantID + "' and ShopID is null and CellID is null and WCID is null and IsDeleted = 0 ;";
                }
        ...
        public List<int> MWO_OverlapCheckerForShopDownwards(int shopid, int mwoID = 0)
        {
            ...
            if (overlappingPlanId2.Count == 0)
            {
                ...
                if (mwoID != 0)
                {
                    sql = "SELECT MWOID FROM i_facility.tblmultipleworkorder where ShopID = '" + ShopID + "'  and CellID is null and WCID is null  and IsDeleted = 0 and MWOID != " + mwoID + " ;";
                }
                else
                {
                    sql = "SELECT MWOID FROM i_facility.tblmultipleworkorder where ShopID = '" + ShopID + "'  and CellID is null and WCID is null  and IsDeleted = 0 ;";
                }
                MySqlDataAdapter da = new MySqlDataAdapter(sql, mc.sqlConnection);
                da.Fill(dataHolder);
                mc.close();
        ...
        public List<int> MWO_OverlapCheckerForCellDownwards(int cellid, int mwoID = 0)
        {
            ...
            if (overlappingPlanId.Count == 0)
            {
                ...
                if (mwoID != 0)
                {
                    sql = "SELECT MWOID FROM i_facility.tblmultipleworkorder where CellID = '" + CellID + "' and WCID is null and IsDeleted = 0 and MWOID != " + mwoID + " ;";
                }
                else
                {
                    sql = "SELECT MWOID FROM i_facility.tblmultipleworkorder where CellID = '" + CellID + "' and WCID is null  and IsDeleted = 0  ;";
                }
                MySqlDataAdapter da = new MySqlDataAdapter(sql, mc.sqlConnection);
                da.Fill(dataHolder);
                mc.close();
        ...
        public List<int> MWO_OverlapCheckerForMachineDownwards(int wc, int mwoID = 0)
        {
            ...
            if (mwoID != 0)
            {
                sql = "SELECT MWOID FROM i_facility.tblmultipleworkorder where IsDeleted = 0 and MWOID != " + mwoID + " and WCID = '" + WorkCenterID + "' ;";
            }
            else
            {
                sql = "SELECT MWOID FROM i_facility.tblmultipleworkorder where IsDeleted = 0 and WCID = '" + WorkCenterID + "' ;";
            }
            MySqlDataAdapter da = new MySqlDataAdapter(sql, mc.sqlConnection);
            da.Fill(dataHolder);
            mc.close();
        ...
        public List<int> MWO_OverlapCheckerForPlant(int plantid, int mwoID = 0)
        {
            ...
            if (mwoID != 0)
            {
                sql = "SELECT MWOID FROM i_facility.tblmultipleworkorder where PlantID = '" + PlantID + "' and ShopID  is null and  CellID  is null and IsDeleted = 0 and MWOID != " + mwoID + " and WCID  is null ;";
            }
            else
            {
                sql = "SELECT MWOID FROM i_facility.tblmultipleworkorder where PlantID = '" + PlantID + "' and ShopID  is null and  CellID  is null and IsDeleted = 0 and WCID  is null ;";
            }
            MySqlDataAdapter da = new MySqlDataAdapter(sql, mc.sqlConnection);
            da.Fill(dataHolder);
            mc.close();
        ...
        public List<int> MWO_OverlapCheckerForShop(int shopid, int mwoID = 0)
        {
            ...
            if (overlappingPlanId.Count == 0)
            {
                ...
                if (mwoID != 0)
                {
                    sql = "SELECT MWOID FROM i_facility.tblmultipleworkorder where ShopID = '" + ShopID + "' and  CellID is null and IsDeleted = 0 and MWOID != " + mwoID + " and WCID  is null ;";
                }
                else
                {
                    sql = "SELECT MWOID FROM i_facility.tblmultipleworkorder where ShopID = '" + ShopID + "' and  CellID  is null and IsDeleted = 0 and WCID  is null ;";
                }
                MySqlDataAdapter da = new MySqlDataAdapter(sql, mc.sqlConnection);
                da.Fill(dataHolder);
                mc.close();
        ...
        public List<int> MWO_OverlapCheckerForCell(int cellid, int mwoID = 0)
        {
            ...
            if (overlappingPlanId.Count == 0)
            {
                ...
                if (mwoID != 0)
                {
                    sql = "SELECT MWOID FROM i_facility.tblmultipleworkorder where CellID = '" + CellID + "' and IsDeleted = 0 and MWOID != " + mwoID + " and WCID is null ;";
                }
                else
                {
                    sql = "SELECT MWOID FROM i_facility.tblmultipleworkorder where CellID = '" + CellID + "' and IsDeleted = 0 and WCID is null ;";
                }
                MySqlDataAdapter da = new MySqlDataAdapter(sql, mc.sqlConnection);
                da.Fill(dataHolder);
                mc.close();
        ...
        public List<int> MWO_OverlapCheckerForMachine(int wc, int mwoID = 0)
        {
            ...
            if (overlappingPlanId2.Count == 0)
            {
                ...
                if (mwoID != 0)
                {
                    sql = "SELECT MWOID FROM i_facility.tblmultipleworkorder where IsDeleted = 0 and MWOID != " + mwoID + " and WCID = '" + MachineID + "' ;";
                }
                else
                {
                    sql = "SELECT MWOID FROM i_facility.tblmultipleworkorder where IsDeleted = 0 and WCID = '" + MachineID + "' ;";
        ...
        public ActionResult ExportEnabledMultiWODetails()
        {
            ...
            var BreakDownLossesData = db.tblmultipleworkorders.Where(m => m.IsDeleted == 0).OrderBy(m => m.PlantID).ThenBy(m => m.ShopID).ThenBy(m => m.CellID).ToList();
            int i = 2;
        ...
        public ActionResult Delete(int id)
        {
            ...
            //int UserID = Convert.ToInt32(Session["UserId"]);
            //String Username = Session["Username"].ToString();
            tblmultipleworkorder tblmwo = db.tblmultipleworkorders.Find(id);
            if(tblmwo == null)
            {
                return HttpNotFound();
            }
            ...
            db.Entry(tblmwo).State = EntityState.Modified;
            db.SaveChanges();
            return RedirectToAction("Index");
        }
        ...
        public bool ValidationCheckForInsertion(int plantID, int shopID,int cellid, int Wcid,int mwoid)
        {
            ...
            if (mwoid == 0)
            {
                var countQuery = db.tblmultipleworkorders.Where(m => m.PlantID == plantID && m.ShopID == shopID && m.CellID == cellid && m.WCID == Wcid && m.IsDeleted == 0 && m.MWOID == mwoid).ToList();
                varcount = countQuery.Count();
            }
            else
            {
                var countQuery = db.tblmultipleworkorders.Where(m => m.MWOID == mwoid && m.PlantID == plantID && m.ShopID == shopID && m.CellID == cellid && m.WCID == Wcid && m.IsDeleted == 0).ToList();
                varcount = countQuery.Count();
            }
        ...
        public ActionResult ImportMultipleWorkorder(HttpPostedFileBase file, string UploadType)
        {
            ...
            if (Request.Files["file"].ContentLength > 0)
            {
                ...
                var unitsItem = db.tblmultipleworkorders.Where(m => m.IsDeleted == 0).ToList();
                ...
                if (UploadType == "OverWrite") // Accept only New Codes
                {
                    ...
                    for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                    {
                        ...
                        {
                            var MasterStdPWTData = db1.tblmultipleworkorders.Where(m => m.MultipleWOName != MultiWOName ).FirstOrDefault();
                            ...
                            if (MasterStdPWTData != null)
                            {
                                try
                                {
                                    ...
                                    try
                                    {
                                         ...
                                         PlantID = Convert.ToInt32(db.tblplants.Where(m => m.PlantName == PlantName && m.IsDeleted == 0).Select(m=>m.PlantID).FirstOrDefault());
                                    ...
                                    try
                                    {
                                        ...
                                        ShopID = Convert.ToInt32(db.tblshops.Where(m => m.ShopName == ShopName && m.IsDeleted == 0).Select(m => m.ShopID).FirstOrDefault());
                                    ...
                                    try
                                    {
                                        ...
                                        CellID = Convert.ToInt32(db.tblcells.Where(m => m.CellName == CellName && m.IsDeleted == 0).Select(m => m.CellID).FirstOrDefault());
                                    ...
                                    try
                                    {
                                        ...
                                        MachineID = Convert.ToInt32(db.tblmachinedetails.Where(m => m.MachineName == MachineName && m.IsDeleted == 0).Select(m => m.MachineID).FirstOrDefault());
                        ...
                        db.tblmultipleworkorders.Add(tblmp);
                        db.SaveChanges();
                    }
                    #endregion
                }
                else if (UploadType == "Update") // OverWrite Existing Values 
                {
                    ...
                    for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                    {
                        ...
                        else
                        {
                            ...
                            var MasterStdPWTData = db.tblmultipleworkorders.Where(m => m.MultipleWOName == MultiWOName).FirstOrDefault();
                ...
                //                        var Dupdata = db.tblmultipleworkorders.Where(m => m.MultipleWOName == MultiWOName && m.IsDeleted == 0).FirstOrDefault();
                //                        if (Dupdata != null)
                //                        {
                //                            Dupdata.IsDeleted = 1;
                ...
                else if (UploadType == "New") // Delete Duplicate and Insert New. // if not Duplicate insert that
                {
                    ...
                    for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                    {
                        ...
                        var mwoid = Convert.ToInt32(db.tblmultipleworkorders.Where(m => m.MultipleWOName == MultiWOName && m.IsDeleted == 0).Select(m=>m.MWOID).FirstOrDefault());
                        var plantID = Convert.ToInt32(db.tblplants.Where(m => m.PlantName == PlantName && m.IsDeleted == 0).Select(m=>m.PlantID).FirstOrDefault());
                        var shopID = Convert.ToInt32(db.tblshops.Where(m => m.ShopName == ShopName && m.IsDeleted == 0).Select(m=>m.ShopID).FirstOrDefault());
                        var cellid = Convert.ToInt32(db.tblcells.Where(m => m.CellName == CellName && m.IsDeleted == 0).Select(m=>m.CellID).FirstOrDefault());
                        var wcid= Convert.ToInt32(db.tblmachinedetails.Where(m => m.MachineName == MachineName && m.IsDeleted == 0).Select(m=>m.MachineID).FirstOrDefault());
                        ...
                        if (check == true)
                        {
                            using (i_facilityEntities1 db1 = new i_facilityEntities1())
                            {
                                    ...
                                    //check for dup and delete previous one.
                                    var Dupdata = db.tblmultipleworkorders.Where(m => m.PlantID == PlantID && m.IsDeleted == 0 && m.ShopID== ShopID && m.CellID == CellID && m.WCID == MachineID).FirstOrDefault();
                                        if (Dupdata != null)
                                        {
                                    Dupdata.IsDeleted = 1;
