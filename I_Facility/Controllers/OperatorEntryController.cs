...
using static I_Facility.GetMode;

namespace I_Facility.Controllers
{
    public class OperatorEntryController : Controller
    {
        public i_facilityEntities1 _UWcontext = new i_facilityEntities1();
        //public CCSOEEServerEntities _ServerContext = new CCSOEEServerEntities();
        public i_facilityEntities1 _ServerContext = new i_facilityEntities1();
        ...
        public ActionResult DashboardProduction()
        {
            ...
            foreach (var item in machinedet)
            {
                ...
                if (GetDashboardData != null)
                {
                    ...
                    I_Facility.ServerModel.tbloperatordashboard TOD = new I_Facility.ServerModel.tbloperatordashboard();
        ...
        //[HttpGet]
        public ActionResult EntryWindow()
        {
            if ((Session["UserId"] == null) || (Session["UserId"].ToString() == String.Empty))
            {
                return RedirectToAction("Login", "Login", null);
            ...
            foreach (var item in machinedet)
            {
                ...
                ViewBag.HoldReason = new SelectList(_UWcontext.tblholdcodes.Where(m => m.IsDeleted == 0), "HoldCodeID", "HoldCodeDesc");
                //ViewData["HoldReason"] = new SelectList(_UWcontext.tblholdcodes.Where(m => m.IsDeleted == 0), "HoldCodeID", "HoldCodeDesc");
                var WOE = _UWcontext.tblworkorderentries.Where(m => m.IsFinished == 0 && m.IsStarted == 1 && m.MachineID == MachineID).OrderByDescending(m => m.HMIID).ToList().FirstOrDefault();
...

                {
                    no = 2;
                    ShiftID = WOE.ShiftID;
                    Operatorid = WOE.OperatorID;
                    ViewData["Operatorid"] = Operatorid;
                    ViewData["Display"] = no;
                    cpID = Convert.ToInt32(WOE.FGCode);

                    //    using (MsqlConnection mc = new MsqlConnection())
                    //    {
                    //        mc.open();

                    //        String getparametersquery = "SELECT Part_No, ProdOrderQty FROM i_facility.i_facility.tblworkorderentry WHERE HMIID = " + WOE.HMIID + "";
                    //        MySqlDataAdapter da = new MySqlDataAdapter(getparametersquery, mc.sqlConnection);
                    ...
                    //        if (dt.Rows.Count != 0)
                    //        {
                    //            WOE.Part_No = dt.Rows[0][0].ToString();
                    //            WOE.ProdOrderQty = Convert.ToInt32(dt.Rows[0][1]);
                    //        }
                    //    }
                    ViewData["ShiftID"] = new SelectList(_UWcontext.shift_master.Where(m => m.IsDeleted == 0), "ShiftID", "ShiftName", ShiftID);
                    ViewData["cpID"] = new SelectList(_UWcontext.tblcellparts.Where(m => m.IsDeleted == 0 && m.CellID == cellid), "cpID", "partNo", cpID);
                    ViewBag.WOSTART = WOE.WOStart;
                    return View(WOE);
                }
        ...
        public string EntryWindowData(string[] Operator, int PartNo, int Shift)
        {
            ...
            foreach (var item1 in machinedet)
            {
                ...
                DateTime correctedDate = DateTime.Now;
                I_Facility.ServerModel.tbldaytiming StartTime = _UWcontext.tbldaytimings.Where(m => m.IsDeleted == 0).FirstOrDefault();
                TimeSpan Start = StartTime.StartTime;
                if (Start.Hours <= DateTime.Now.Hour)
                {
               ...
               var Partnumdet= _UWcontext.tblcellparts.Find(PartNo);
                ...
                I_Facility.ServerModel.tblworkorderentry WOEntry = new I_Facility.ServerModel.tblworkorderentry();
                ...
                WOEntry.Yield_Qty = 0;
                WOEntry.IsStarted = 1;
                WOEntry.CorrectedDate = correctedDate.ToString("yyyy-MM-dd");
        ...
        public string StoreRejectVal(int Shift, string[] rejectval, string[] ReasonID)
        {
            int i = 0;
            int curr = DateTime.Now.Hour;
            int currnext = curr + 1;
            ...
            var celliddet = Session["CellID"];
            int cellid = Convert.ToInt32(celliddet);
            int machineid = _UWcontext.tblmachinedetails.Where(m => m.CellID == cellid && m.IsDeleted == 0 && m.IsLastMachine == 1).Select(m => m.MachineID).FirstOrDefault();
            int wodet = _UWcontext.tblworkorderentries.Where(m => m.ShiftID == Shift && m.CorrectedDate == correcteddate && m.MachineID == machineid).Select(m => m.HMIID).FirstOrDefault();
