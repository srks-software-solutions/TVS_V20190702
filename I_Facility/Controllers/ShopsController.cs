...
namespace I_Facility.Controllers
{
    public class ShopsController : Controller
    {
        ...
        // Update Department
        [HttpPost]
        public ActionResult EditShops(ShopModel shop, int PlantID = 0)
        {
            if ((Session["UserId"] == null) || (Session["UserId"].ToString() == String.Empty))
            {
                return RedirectToAction("Login", "Login", null);
            ...
            using (i_facilityEntities1 db = new i_facilityEntities1())
            {
                ...
                if (doesThisdeptExists.Count == 0)
                {
                    ...
                    Department.NoOfMachines = shop.Shops.NoOfMachines;
                    Department.PlantID = PlantID;
        ...
        public ActionResult DeleteShops(int id)
        {
            ...
            {
                cellrow.IsDeleted = 1;
                db.Entry(cellrow).State = EntityState.Modified;
                db.SaveChanges();
        ...
        {
            var data = db.tblshops.Where(m => m.ShopID == Id).Select(m => new { PlantId = m.PlantID, shopname = m.ShopName, shopdesc = m.ShopDesc, shopdisplay = m.Shopdisplayname, NoOfMachine = m.NoOfMachines });
